...
#include <string.h>
#include <math.h>
void _CHANGE_STR(char *str,double *val);
int main(void){
  FILE *file;
  int i,j,k,v,count=0,ncent=0,*pos,nlines,l=0;
  int icent,n,nbasis,off,n_orb,n_occu_orb,xgrid,ygrid,zgrid;
  int *ns_icent,*np_icent,*nd_icent,*nf_icent;
  int *aux_s_icent,*aux_p_icent,*aux_d_icent,*aux_f_icent;
  int **nprim_s_icent,**nprim_p_icent,**nprim_d_icent,**nprim_f_icent;
  double val1,val2,val,a,**center_xyz,**mo_occu,*occu,fact,S_xyz,x,y,z,***mo_orb,*we;
  double **coeff_s,**expon_s,**coeff_p,**expon_p,**coeff_d,**expon_d,**coeff_f,**expon_f;
  char str[50],str1[200],str2[200],command[500],fichero[400];
  const double AnsgAu=1.889726124565062;
  scanf("%s",fichero);
  scanf("%d",&l);
  strcpy(command,"./read.sh ");
  ...
  for(i=0;i<ncent;i++){
    ...
    printf("Position of Nuclei %d x=% .8le y=% .8le z=% .8le\n",i+1,center_xyz[i][0],center_xyz[i][1],center_xyz[i][2]);
  }
  fclose(file);
  ...
  mo_orb=(double***)calloc(200,sizeof(double));
  for(i=0;i<200;i++){
    mo_orb[i]=(double**)calloc(200,sizeof(double));
    for(j=0;j<200;j++){
      mo_orb[i][j]=(double*)calloc(200,sizeof(double));
    }
  }
  for(xgrid=0;xgrid<200;xgrid++){
	...
	v=0;
	for(i=0;i<ncent;i++){
	  x=(-6.0+xgrid*0.06)-center_xyz[i][0];
	  y=(-6.0+ygrid*0.06)-center_xyz[i][1];
	  z=(-6.0+zgrid*0.06)-center_xyz[i][2];
	  off=0;
