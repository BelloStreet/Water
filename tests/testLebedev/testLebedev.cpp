...
//
{
  ...
  for (rule = 1; rule <= rule_max; rule++) {
    ...
    cout << "  " << setw(4) << rule << "  " << setw(4) << available << "  "
         << setw(4) << order << "  " << setw(4) << precision << "\n";
  }
...
#define mmax ((nmax * 2 + 3) * (nmax * 2 + 3) / 3)
  ...
  for (n = 1; n <= nmax; n++) {
    ...
    if (available) {
      ...
      //
      //  For each abscissa X(M), compute the values 1, X(M)^2, X(M)^4, ...,
      //  X(M)^2*N.
      //
      for (m = 0; m < order; m++) {
        xn[m * (n + 1)] = 1.0;
      ...
      for (i = 0; i <= n; i++) {
        for (j = 0; j <= n - i; j++) {
          ...
          integral_approx = 0.0;
          for (m = 0; m < order; m++) {
            integral_approx = integral_approx + w[m] * xn[i + m * (n + 1)] *
                                                    yn[j + m * (n + 1)] *
                                                    zn[k + m * (n + 1)];
          }
          //
      ...
      cout << "  Order = " << setw(4) << order
           << "  LMAXW = " << precision_table(n) << "  max error = " << err_max
           << "\n";
      //
