...
// void GRID::_HH1(double Zcharge,int id,int numprocs,int norb,orbitals *p,cmplx
// *NE,cmplx *H1){
...
void GRID::_HH1(double Zcharge, int id, int numprocs, int norb, orbitals *p,
                cmplx *NE, cmplx *H1) {
  int i, j, i1, j1, li, lf, local_n, lmorb, io;
  double z1, *A, *B;
  gsl_matrix_view m;
    ...
    for (j = 0; j < nbreak * lmorb; j++) {
      ...
      if (li == lf) {
        A[j + lmorb * nbreak * (i + id * local_n)] =
            creal(KE[j1 + nbreak * i1 + nbreak * nbreak * p[2].lo[li]]);
      }
      for (io = 0; io < norb; io++) {
        A[j + lmorb * nbreak * (i + id * local_n)] =
            A[j + lmorb * nbreak * (i + id * local_n)] +
            2.0 * creal(p[io].J[j + lmorb * nbreak * (i + id * local_n)]) -
            creal(p[io].K[j + lmorb * nbreak * (i + id * local_n)]);
      }
      A[j + lmorb * nbreak * (i + id * local_n)] =
          A[j + lmorb * nbreak * (i + id * local_n)] +
          creal(NE[j + lmorb * nbreak * (i + id * local_n)]);
    }
  }
  ...
  for (io = 0; io < norb; io++) {
    ...
    for (i = 0; i < local_n; i++) {
      ...
      for (j = 0; j < lmorb * nbreak; j++) {
        z1 = z1 +
             A[j + lmorb * nbreak * (i + id * local_n)] * creal(p[io].cilm[j]);
      }
      B[i + id * local_n] = z1;
    }
    MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, B, local_n, MPI_DOUBLE,
                  MPI_COMM_WORLD);
    if (id == 0) {
      z1 = 0.0;
      for (i = 0; i < lmorb * nbreak; i++) {
  ...
  if (id == 0) {
    ...
    for (i = 0; i < lmorb * nbreak; i++) {
      for (j = 0; j < lmorb * nbreak; j++) {
        k = j + lmorb * nbreak * i;
        gsl_vector_set(v1, k,
                       fabs(creal(p[io].K[i + nbreak * lmorb * j]) -
                            creal(p[io].K[j + nbreak * lmorb * i])));
        gsl_vector_set(v2, k,
                       fabs(creal(p[io].J[i + nbreak * lmorb * j]) -
                            creal(p[io].J[j + nbreak * lmorb * i])));
      }
    }
...
void GRID::_KO(int id, int numprocs, int lmerb, int *le, int lmorb, int *lo,
               cmplx *cilm, cmplx *C3j, cmplx *C3i, cmplx *K) {
    ...
    for (j = 0; j < nbreak * lmerb; j++) {
      ...
      for (l = 0; l < nsph; l++) {
        ...
        for (l1 = 0; l1 < lmorb; l1++) {
          if (la[l] >= abs(lo[l1] - le[li]) &&
              la[l] <= lo[l1] + le[li]) {  // esto es nuevo
            z3 = C3j[l + nsph * (l1 + lmorb * li)];
            for (l2 = 0; l2 < lmorb; l2++) {
              if (la[l] >= abs(lo[l2] - le[lf]) &&
                  la[l] <= lo[l2] + le[lf]) {  // esto es nuevo
  ...
  MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, K, local_n * nbreak * lmerb,
...
void GRID::_JO(int id, int numprocs, int lmerb, int *le, int lmorb, int *lo,
               cmplx *cilm, cmplx *C3j, cmplx *C3i, cmplx *J) {
  int i, j, k, l, nsph, i1, i2, j1, j2, l1, l2, m1, m2, li, lf, local_n;
  int lmax, nchannels, *la, *ma;
  double start_time, end_time;
  cmplx beta, z1, z2, z3, z4, z5;
  ...
  for (i = 0; i < local_n; i++) {
    ...
    for (j = 0; j < nbreak * lmerb; j++) {
      ...
      if (i1 == j1) {
        for (k = 0; k < nbreak; k++) {
          for (l = 0; l < nsph; l++) {
            if (la[l] >= abs(le[li] - le[lf]) &&
                la[l] <= le[li] + le[lf]) {  // esto es nuevo
              z2 = 4.0 * M_PI * pow(-1, ma[l]) * _PSSN(i1, k, la[l]) /
                   (2.0 * la[l] + 1.0);
              z3 = C3j[l + nsph * (lf + lmerb * li)];
              for (l1 = 0; l1 < lmorb; l1++) {
                for (l2 = 0; l2 < lmorb; l2++) {
                  if (la[l] >= abs(lo[l1] - lo[l2]) &&
                      la[l] <= lo[l1] + lo[l2]) {  // esto es nuevo
  ...
  MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, J, local_n * nbreak * lmerb,
                MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD);
  if (id == 0) {
    end_time = MPI_Wtime();
    printf("% .8le\n", end_time - start_time);
...
void GRID::_NE(int id, int numprocs, int lmerb, int *le, cmplx *C3j, cmplx *C3i,
               cmplx *NE) {
  FILE *file;
  int i, j, k, l, nsph, i1, i2, j1, j2, l1, l2, m1, m2, li, lf, local_n;
  ...
  for (i = 0; i < ncent; i++) {
    R[i] = sqrt(pow(center_xyz[i][0], 2) + pow(center_xyz[i][1], 2) +
                pow(center_xyz[i][2], 2));
    for (l = 0; l < lmax; l++) {
      for (k = 0; k < nbreak; k++) {
        xi = creal(Xz[k]);
        if (xi < R[i]) {
        ...
        for (k = 0; k < ncent; k++) {
          ...
          if (k == 0) {
            ...
            for (l = 0; l < nsph; l++) {
              if (la[l] >= abs(le[li] - le[lf]) &&
                  la[l] <= le[li] + le[lf]) {  // esto es nuevo
                z2 = 4.0 * M_PI * In[k][i1][la[l]] *
                     C3j[l + nsph * (lf + lmerb * li)] / (2.0 * la[l] + 1.0);
  ...
  MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, NE,
                local_n * nbreak * lmerb, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD);
  if (id == 0) {
    end_time = MPI_Wtime();
    printf("% .8le\n", end_time - start_time);
    fflush(stdout);
...
void GRID::_GETORB(char *orbfile, char *symm, char *irrep, int lo, char *str1,
  ...
  *cilm = (cmplx *)calloc(nbreak * nchannels, sizeof(cmplx));
    ...
    for (i = 0; i < nbreak; i++) {
      ...
      for (j = 0; j < n; j++) {
        ...
        if (strcmp(symm, "ylm") == 0) {
          beta = beta + cpow(Wz[i], 0.5) * wi[j] * orb[j + n * i] *
                            conj(_Ylm((*li)[l], (*mi)[l], theta, phi)) * Xz[i];
        } else {
          beta = beta + cpow(Wz[i], 0.5) * wi[j] * orb[j + n * i] *
                            _Blm(str, (*li)[l], (*mi)[l], theta, phi) * Xz[i];
        }
      }
  ...
  // diatomics strcpy(fichero,"orbital_"); sprintf(sto,"%d",lo);
  // strcat(fichero,sto);
  // strcat(fichero,".inp");
  ...
  for (i = nbreak - 1; i >= 0; i--) {
    beta = 0.0;
    for (l = 0; l < nchannels; l++) {
      beta = beta + (*cilm)[i + nbreak * l] *
                        _Blm(str, (*li)[l], (*mi)[l], M_PI, 0.0) /
                        (cpow(Wz[i], 0.5) * Xz[i]);
    }
    fprintf(file, "% .8le % .8le % .8le\n", -creal(Xz[i]), creal(beta),
            cimag(beta));
  }
  for (i = 0; i < nbreak; i++) {
    beta = 0.0;
