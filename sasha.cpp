...
void _C3jBlm(char *str1, char *str2, int j1, int j2, int j3, int m1, int m2,
             int m3, cmplx *zeta, cmplx *beta) {
  int i, j, k, l;
  ...
  *beta = 0.0 + 0.0 * I;
  for (j = 0; j < n; j++) {
    ...
    if (strcmp(str1, "y") == 0 && strcmp(str2, "y") == 0) {
      *zeta = *zeta + wi[j] * conj(_Ylm(j1, m1, theta, phi)) *
                          _Ylm(j2, m2, theta, phi) * _Ylm(j3, m3, theta, phi);
      *beta = *beta + wi[j] * conj(_Ylm(j1, m1, theta, phi)) *
                          _Ylm(j2, m2, theta, phi) * _Ylm(j3, -m3, theta, phi);
    } else {
      *zeta = *zeta + wi[j] * _Blm(str1, j1, m1, theta, phi) *
                          _Blm(str2, j2, m2, theta, phi) *
                          _Ylm(j3, m3, theta, phi);
...
void _GETC3J(int id, int lmax, char *str1, char *str2, int lmorb1, int lmorb2,
             int *lo1, int *mo1, int *lo2, int *mo2, cmplx **C3Kj, cmplx **C3Ki,
             cmplx **C3Jj, cmplx **C3Ji, int type) {
  ...
  *C3Ji = (cmplx *)calloc(lmorb2 * lmorb2 * nsph, sizeof(cmplx));
      ...
      for (j = 0; j < lmorb2; j++) {
        ...
        for (lv = 0; lv < lmax; lv++) {
          if (la[k] >= abs(lo1[i] - lo2[j]) && la[k] <= lo1[i] + lo2[j]) {
            for (mv = 0; mv <= lv; mv++) {
              if (mv == 0) {
                _C3jBlm(str1, str2, lo1[i], lo2[j], la[k], mo1[i], mo2[j],
                        ma[k], &zeta, &beta);
      ...
      for (j = i; j < lmorb1; j++) {
        ...
        for (lv = 0; lv < lmax; lv++) {
          if (la[k] >= abs(lo1[i] - lo1[j]) && la[k] <= lo1[i] + lo1[j]) {
            for (mv = 0; mv <= lv; mv++) {
              if (mv == 0) {
                _C3jBlm(str1, str1, lo1[i], lo1[j], la[k], mo1[i], mo1[j],
                        ma[k], &zeta, &beta);
      ...
      for (j = i; j < lmorb2; j++) {
        ...
        for (lv = 0; lv < lmax; lv++) {
          if (la[k] >= abs(lo2[i] - lo2[j]) && la[k] <= lo2[i] + lo2[j]) {
            for (mv = 0; mv <= lv; mv++) {
              if (mv == 0) {
                _C3jBlm(str2, str2, lo2[i], lo2[j], la[k], mo2[i], mo2[j],
                        ma[k], &zeta, &beta);
  ...
  MPI_Bcast(&((*C3Kj)[0]), lmorb1 * lmorb2 * nsph, MPI_DOUBLE_COMPLEX, 0,
            MPI_COMM_WORLD);
  MPI_Bcast(&((*C3Ki)[0]), lmorb1 * lmorb2 * nsph, MPI_DOUBLE_COMPLEX, 0,
            MPI_COMM_WORLD);
  MPI_Bcast(&((*C3Jj)[0]), lmorb1 * lmorb1 * nsph, MPI_DOUBLE_COMPLEX, 1,
            MPI_COMM_WORLD);
  MPI_Bcast(&((*C3Ji)[0]), lmorb2 * lmorb2 * nsph, MPI_DOUBLE_COMPLEX, 1,
            MPI_COMM_WORLD);
