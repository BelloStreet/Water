...
subroutine build_KEmats()
  ...
  complex*16 zImat(nbas, nbas),zR1(nbas,nbas),fval
  ...
  ! ! bueno esto tb es nuevo
  ! do lam=0,lamax
  !    do i=1, nbas
  !       do j=1, nbas
  !          fval=dble(2*lam+1)*TIXX(j, i, lam)/(xz(j)*xz(i)*sqrt(wz(j)*wz(i)))
  !          fval=fval+((xz(j)*xz(i))**(lam))/((az(nel))**(2*lam+1))
  !          TIXX(j, i, lam) = fval
  !       end do
  !    end do
  ! end do
