...
void GRID::_HH1(double Zcharge,int id,int numprocs,int lmorb,int *lo,int *mo,cmplx *K,cmplx *J,cmplx *H1){
  ...
  for(i=0;i<local_n;i++){
    ...
    for(j=0;j<nbreak*lmorb;j++){
      ...
      j1=j-nbreak*lf;
      if(li==lf){
	A[j+lmorb*nbreak*(i+id*local_n)]=creal(KE[j1+nbreak*i1+nbreak*nbreak*lo[li]]);
	if(i1==j1){
	  A[j+lmorb*nbreak*(i+id*local_n)]=A[j+lmorb*nbreak*(i+id*local_n)]-Zcharge/creal(Xz[i1]);
	}
      }
      A[j+lmorb*nbreak*(i+id*local_n)]=A[j+lmorb*nbreak*(i+id*local_n)]+creal(2.0*J[j+lmorb*nbreak*(i+id*local_n)]-K[j+lmorb*nbreak*(i+id*local_n)]);
    }
  }
...
void GRID::_JMK(int id,int numprocs,int lmorb,cmplx *cilm,cmplx *K,cmplx *J){
  int i,j,i1,j1,li,lf,local_n;
  cmplx z1,*A;
  ...
  for(i=0;i<local_n;i++){
    ...
    for(j=0;j<lmorb*nbreak;j++){
      z1=z1+(J[j+lmorb*nbreak*(i+id*local_n)]-K[j+lmorb*nbreak*(i+id*local_n)])*cilm[j];
    }
    A[i+id*local_n]=z1;
  }
  ...
  if(id==0){
    ...
    for(i=0;i<lmorb*nbreak;i++){
      for(j=0;j<lmorb*nbreak;j=j+60){
      	printf("% .8le % .8le % .8le % .8le\n",creal(K[i+nbreak*lmorb*j]),creal(K[j+nbreak*lmorb*i]),creal(J[i+nbreak*lmorb*j]),creal(J[j+nbreak*lmorb*i]));
      }
    }
  }
  free(A);
...
void GRID::_KO(int id,int numprocs,char *str,int lmorb,int *lo,int *mo,cmplx *cilm,cmplx *K){
  ...
  for(i=0;i<lmorb;i++){
	...
	C3i[k+nsph*(j+lmorb*i)]=_C3jBlm(str,str,lo[i],lo[j],la[k],mo[i],mo[j],-ma[k]);
      }
    }
  }
  local_n=(int)(floor(nbreak*lmorb/numprocs));
  ...
  MPI_Allgather(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,K,local_n*nbreak*lmorb,MPI_DOUBLE_COMPLEX,MPI_COMM_WORLD);
  if(id==0){
    end_time=MPI_Wtime();
    printf("% .8le\n",end_time-start_time);
    fflush(stdout);
...
void GRID::_JO(int id,int numprocs,char *str,int lmorb,int *lo,int *mo,cmplx *cilm,cmplx *J){
  int i,j,k,l,nsph,i1,i2,j1,j2,l1,l2,m1,m2,li,lf,local_n;
  int lmax,nchannels,*la,*ma;
  double *C3j,*C3i,start_time,end_time;
  cmplx beta,z1,z2,z3,z4,z5;
  ...
  MPI_Allgather(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,J,local_n*nbreak*lmorb,MPI_DOUBLE_COMPLEX,MPI_COMM_WORLD);
  if(id==0){
    end_time=MPI_Wtime();
    printf("% .8le\n",end_time-start_time);
    fflush(stdout);
