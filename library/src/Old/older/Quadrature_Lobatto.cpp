...
Quadrature_Lobatto::Quadrature_Lobatto(int &a_dvr) : Quadrature(a_dvr) {
  ...
  endpts[0] = -1.0;
  endpts[1] = 1.0;
  gaussq_(&m_kind, &a_dvr, &m_alpha, &m_beta, &m_kpts, endpts, src, x, w);
  m_points.reserve(a_dvr);
  m_weights.reserve(a_dvr);
  for (int i = 0; i < a_dvr; ++i) {
    m_points.push_back(std::move(x[i]));
    m_weights.push_back(std::move(w[i]));
  }
  ...
  delete[] src;
  delete[] endpts;
  test = std::make_unique<double[]>(5);
  for (int i = 0; i < 5; ++i) test[i] = i;
}

Quadrature_Lobatto::~Quadrature_Lobatto(){};

double *Quadrature_Lobatto::getUnique() { return test.get(); }

std::vector<double> Quadrature_Lobatto::getPoints() const { return m_points; }

std::vector<double> Quadrature_Lobatto::getWeights() const { return m_weights; }
