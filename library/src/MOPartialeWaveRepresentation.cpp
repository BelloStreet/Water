      ...
      for (int radial_point = 0; radial_point < a_femdvr_grid->getNbas();
           ++radial_point)
        {
          ...
          for (int angular_point = 0;
               angular_point < a_angular_grid->getAngularOrder();
               ++angular_point)
            {
              ...
              beta +=
                sqrt(a_femdvr_grid->getWeight(radial_point).real()) *
...
size_t
MOPartialWaveRepresentation::getNumChannels() const
{
  return m_num_channels;
}

std::string
MOPartialWaveRepresentation::getType() const
{
  return m_type;
}

int
MOPartialWaveRepresentation::getL(int index) const
{
  return m_quantum_number_l[index];
}

int
MOPartialWaveRepresentation::getM(int index) const
{
  return m_quantum_number_m[index];
}

double
MOPartialWaveRepresentation::Blm(const std::string type,
                                 const int         l,
                                 const int         m,
                                 const double      theta,
                                 const double      phi)
