...
Joperator::Joperator(std::shared_ptr<FEMDVR> a_femdvr_grid,
  ...
  for (int l = 0; l < a_angular_grid->getLmax(); ++l) {
    ...
    /*     LAPACK_COL_MAJOR, a_femdvr_grid->getNbas(), a_femdvr_grid->getNbas(),
     */
    /*     reinterpret_cast<_Complex double *>(tmp_laplacian.get()), */
    /*     a_femdvr_grid->getNbas(), tmp_ipiv.get(), */
    /*     reinterpret_cast<_Complex double *>(tmp_swap_value.get()), */
    /*     a_femdvr_grid->getNbas()); */
    for (int i = 0; i < a_femdvr_grid->getNbas(); ++i) {
      for (int j = 0; j < a_femdvr_grid->getNbas(); ++j) {
...
std::complex<double> Joperator::getTXX(int index) const {
  return m_dvr_rep[index];
}

std::complex<double> Joperator::getTIXX(int index) const {
