                     ...
                     const int &             a_lmax_times_2)
{
  ...
  for (int l = 0; l < a_lmax_times_2; ++l)
    {
      auto tmp_laplacian =
        std::make_unique<std::complex<double>[]>(nbas * nbas);
      auto tmp_swap_value =
        std::make_unique<std::complex<double>[]>(nbas * nbas);
      for (int i = 0; i < nbas; ++i)
        {
      ...
      auto info =
        LAPACKE_zgesv(LAPACK_COL_MAJOR,
                      nbas,
                      nbas,
      ...
      for (int i = 0; i < nbas; ++i)
        {
          std::complex<double> tmp_i_factor =
            1.0 / (a_femdvr_grid->getPoint(i) *
                   pow(a_femdvr_grid->getWeight(i), 0.5));
          ...
          for (int j = 0; j < nbas; ++j)
            {
              ...
              tmp_value =
                tmp_value +
                (pow(a_femdvr_grid->getPoint(i) * a_femdvr_grid->getPoint(j),
                     l)) /
...
std::complex<double>
Toperator::getTXX(int index) const
{
  return m_dvr_rep[index];
}

std::complex<double>
Toperator::getTIXX(int index) const
{
