...
public:
  ...
  std::complex<double> getPartialWaveRep(int index) const;

  /// Getter for this orbital's BLM
  double getBLM(int index) const;
...
private:
  int m_lmax, m_num_channels;
  std::unique_ptr<double[]> m_blm;
