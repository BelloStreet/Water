...
public:
  ...
  */
  ...
  /// Getter for points.
  std::complex<double> *getPoints() const;

  /// Getter for weights.
  std::complex<double> *getWeights() const;

  /// Getter for laplacian.
  std::complex<double> *getLaplacian() const;

  /// Get number of basis functions
  const size_t &getNbas();
...
private:
  ...
  // Helper function to find the derivative
  std::unique_ptr<std::complex<double>[]>
  derivative(const int, const std::complex<double> *);
};
#endif
