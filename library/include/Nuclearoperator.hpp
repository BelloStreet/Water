#ifndef _NUCLEAROPERATOR_HPP_
#define _NUCLEAROPERATOR_HPP_
...
public:
  ...
  /// term.
  Nuclearoperator(const size_t &a_Nbas,
  ...
  Nuclearoperator(const int &a_numprocs, const int &id, const size_t &a_Nbas,
                  std::shared_ptr<AngularGrid> a_angular_grid,
                  std::shared_ptr<MOPartialWaveRepresentation> a_ket_orbital,
                  std::shared_ptr<MOPartialWaveRepresentation> a_bra_orbital,
                  std::shared_ptr<Toperator> a_T);

  Nuclearoperator(const int &a_numprocs,
  ...
  std::complex<double> getJ(int index) const;
